...
description: 使用中央 skills 目录和项目级软链接管理 Agent skills。当用户需要检查、初始化、下载/克隆、链接、批量链接、取消链接、停用、删除入口、同步、迁移、更新、切换版本、去重或排查 Codex、Claude Code、.agents、.codex、.claude、用户级 skills 目录或项目级 skills 目录时使用。也适用于处理失效软链接、中央 skills 仓库、多 Agent 共享 skills、检查下载的 skills 是否有更新、通过 git 更新 skills、判断是否应该 fork 别人的 skill，以及把复制出来的 skills 安全迁移为软链接。
---
