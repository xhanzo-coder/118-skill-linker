...
4. 识别用户级目录：
   ...
   - `~/.codex/skills`
   - `~/.claude/skills`
5. 识别中央目录候选，但不要把 Agent 全局 skills 目录默认当作中央库：
   ...
   - `~/Skills`
   - 不要默认推荐 `~/.agents/skills`、`~/.codex/skills`、`~/.claude/skills` 作为中央库；这些是用户级 Agent 全局目录，只在用户明确选择时才可作为中央库。
6. 汇报当前目录结构、重复 skill 名、软链接、失效软链接、复制出来的 skills，以及可能的中央仓库。
7. 如果配置中已有中央目录，围绕该中央目录给出建议方案；如果没有配置，优先推荐 `~/.118-skill-linker/AgentSkills`（Windows 为 `%USERPROFILE%\.118-skill-linker\AgentSkills`），并允许用户选择其他非全局目录。
8. 只执行已经确认的操作。
...
首次使用时，先运行 `inspect`，总结发现的问题。如果 `inspect` 返回有效配置，优先使用配置中的中央目录；如果没有有效配置，默认推荐 `~/.118-skill-linker/AgentSkills`（Windows 为 `%USERPROFILE%\.118-skill-linker\AgentSkills`）并写入用户级 `~/.skill-linker.json`。如果用户自定义路径，判断它是否为 Agent 全局目录；如果用户选择 `~/.agents/skills` 这类 Agent 全局目录，说明它不推荐默认作为中央库，并要求明确确认 `确认使用全局 skills 目录作为中央库`。用户确认中央目录且没有特殊项目隔离要求时，默认建议写入用户级 `~/.skill-linker.json`。
