...
7. 如果配置中已有中央目录，围绕该中央目录给出建议方案；如果没有配置但发现用户级中央候选，优先建议用户确认该用户级中央目录并写入用户级配置。
8. 只执行已经确认的操作。
...
没有配置但发现用户级中央候选时，不要默认建议“只整理当前项目级入口”或“保留项目内真实 skill 原件”。除非用户明确要求项目本地模式，否则推荐方案必须是：确认用户级中央目录、写入用户级配置、把项目内真实 skill 迁移或同步到用户级中央目录，再把项目入口改成软链接。
...
6. 如果没有配置但发现用户级中央候选，例如 `~/.agents/skills`，推荐方案必须是确认用户级中央目录并写入用户级 `~/.skill-linker.json`。不要把“只整理当前项目级入口、不迁移到用户级中央库”作为推荐方案。
7. 如果没有配置也没有用户级中央候选，向用户说明候选中央目录，并请用户确认要使用哪一个。
8. 在没有配置且用户未确认中央目录前，不要执行迁移、同步、替换或 git 更新。

如果没有发现明显的中央目录，询问用户希望把中央 skills 目录放在哪里；不要自行创建中央目录，除非用户明确同意。

反例：如果项目里有 `.agents/skills/foo` 真实目录，同时发现用户级中央候选 `~/.agents/skills`，不要回复“我建议先只整理当前项目级入口，不迁移到用户级中央库”。正确做法是把 `~/.agents/skills` 标为推荐中央目录候选，并给出“写入用户级配置 + 迁移或同步项目真实 skill + 创建项目入口软链接”的确认计划。
...
- 用户级中央目录：保存 skill 原件，例如 `/Users/name/.agents/skills`
...
必须明确说明：项目级 `.agents/skills` 只是当前项目的 skills 入口目录，不是用户级中央 skills 库。
...
用户级中央目录：
- [用户级][中央目录][真实目录] ~/.agents/skills
  绝对路径：/Users/name/.agents/skills
...
| 迁移 | [项目级][通用 .agents] /abs/project/.agents/skills/foo | [用户级][中央目录] /Users/name/.agents/skills/foo | foo 原件进入用户级中央目录 | 否 |
| 备份 | [项目级][Claude] /abs/project/.claude/skills | /abs/project/.skill-linker-backup/.../.claude/skills | 保留原目录副本 | 否 |
| 建立软链接 | [项目级][通用 .agents] /abs/project/.agents/skills/foo | /Users/name/.agents/skills/foo | 当前项目入口指向用户级中央原件 | 否 |
...
如果计划会写入用户级配置或迁移到用户级中央目录，确认口令必须体现用户级影响，例如：

```text
如果你确认要写入用户级配置并整理“当前项目级 skills”，请回复：确认写入用户级配置并整理当前项目
```
...
首次使用时，先运行 `inspect`，总结发现的问题。如果 `inspect` 返回有效配置，优先使用配置中的中央目录；如果没有有效配置但发现用户级中央候选，默认推荐用户确认该用户级中央目录并写入用户级 `~/.skill-linker.json`。只有没有有效配置、也没有明确用户级中央候选时，才询问用户哪个目录应作为中央 skills 目录。不要仅凭某个路径存在就把它推断为中央目录。用户确认中央目录且没有特殊项目隔离要求时，默认建议写入用户级 `~/.skill-linker.json`。
...
同步多个 Agent 时，优先使用当前项目 `.agents/skills` 作为项目级入口目录，并且只在用户确认后把 `.claude/skills` 和 `.codex/skills` 链接到它。不要把这个项目级入口目录称为中央 skills 库。如果其中任一目录已经是真实目录，报告冲突并让用户选择处理方式。
