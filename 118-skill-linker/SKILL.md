...
6. 如果用户确认了中央目录且没有特殊项目隔离要求，优先建议写入用户级 `~/.skill-linker.json`，不要默认写当前项目 `.skill-linker.json`。
7. 在没有配置且用户未确认中央目录前，不要执行迁移、同步、替换或 git 更新。
...
首次使用时，先运行 `inspect`，总结发现的问题。如果 `inspect` 返回有效配置，优先使用配置中的中央目录；如果没有有效配置，再询问用户哪个目录应作为中央 skills 目录。不要仅凭某个路径存在就把它推断为中央目录。用户确认中央目录且没有特殊项目隔离要求时，默认建议写入用户级 `~/.skill-linker.json`。
